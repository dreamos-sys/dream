  ...
  return (
    <div className="w-full min-h-screen bg-[#020617] text-white flex flex-col items-center justify-center p-4 md:p-10 font-sans overflow-hidden">
      <header className="mb-10 text-center relative z-10">
        ...
        <p className="text-[10px] text-white/30 tracking-[0.5em] mt-2 uppercase italic text-center">Sovereign Edition • Neural Sync</p>
      </header>
      <div className={`grid gap-4 md:gap-6 w-full max-w-7xl relative z-10 ${isTV ? 'grid-cols-6' : 'grid-cols-3'}`}>
        {modules.map((m, i) => {
          ...
          return (
            <motion.button key={i} whileHover={{ scale: 1.1, backgroundColor: 'rgba(255, 215, 0, 0.1)' }} className="aspect-square flex flex-col items-center justify-center bg-white/5 rounded-[25px] md:rounded-[35px] border border-white/10 shadow-2xl group transition-all duration-300">
              <IconComp className="w-10 h-10 md:w-16 md:h-16 mb-3 text-[#FFD700] group-hover:drop-shadow-[0_0_12px_#FFD700]" />
