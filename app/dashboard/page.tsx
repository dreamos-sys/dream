...
export default function DashboardPage() {
  ...
  const modules = [
    { name: 'Command', icon: '🎯', color: '#8b5cf6', path: '/modules/command' },
    { name: 'Booking', icon: '📅', color: '#3b82f6', path: '/modules/booking' },
    { name: 'K3', icon: '⚠️', color: '#f59e0b', path: '/modules/k3' },
    { name: 'Security', icon: '🛡️', color: '#10b981', path: '/modules/security' },
    { name: 'Janitor', icon: '🧹', color: '#ec4899', path: '/modules/janitor' },
    { name: 'Stok', icon: '📦', color: '#f97316', path: '/modules/stok' },
    { name: 'Maintenance', icon: '🔧', color: '#06b6d4', path: '/modules/maintenance' },
    { name: 'Inventaris', icon: '📋', color: '#6366f1', path: '/modules/inventaris' },
  ...
  const handleNav = (page: string) => {
    ...
    else if (page === 'alerts') router.push('/alerts');    else if (page === 'settings') router.push('/settings');
  };
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#020617', color: 'white', paddingBottom: '100px' }}>
      <header style={{ position: 'sticky', top: 0, background: 'rgba(2,6,23,0.95)', backdropFilter: 'blur(20px)', padding: '20px 15px', borderBottom: '1px solid rgba(16,185,129,0.2)', zIndex: 100 }}>
        ...
        <div onClick={handleGhostTap} style={{ position: 'fixed', top: '15px', right: '15px', background: 'rgba(16,185,129,0.2)', border: '1px solid rgba(16,185,129,0.3)', color: '#10b981', padding: '6px 12px', borderRadius: '20px', fontSize: '9px', fontWeight: '700', cursor: 'pointer', zIndex: 101 }}>👻 Ghost</div>
      </header>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '15px', padding: '15px' }}>
        {modules.map((mod) => (
          <div key={mod.name} onClick={() => router.push(mod.path)} style={{ background: 'rgba(16,185,129,0.05)', border: '1px solid rgba(16,185,129,0.1)', borderRadius: '25px', padding: '25px 15px', textAlign: 'center', cursor: 'pointer' }}>            <div style={{ width: '60px', height: '60px', margin: '0 auto 10px', borderRadius: '15px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '30px', background: `linear-gradient(135deg,${mod.color},#059669)` }}>{mod.icon}</div>
      ...
      <nav style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: '480px', height: '70px', background: 'rgba(2,6,23,0.95)', backdropFilter: 'blur(20px)', borderTop: '1px solid rgba(16,185,129,0.2)', display: 'flex', justifyContent: 'space-around', alignItems: 'center', zIndex: 1000 }}>
        <button onClick={() => handleNav('home')} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', color: activeNav === 'home' ? '#10b981' : 'rgba(148,163,184,0.8)', background: activeNav === 'home' ? 'rgba(16,185,129,0.1)' : 'none', border: 'none', fontSize: '9px', padding: '8px 12px', borderRadius: '12px', cursor: 'pointer' }}><span style={{ fontSize: '20px', marginBottom: '4px' }}>🏠</span><span>Home</span></button>
        <button onClick={() => handleNav('profile')} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', color: activeNav === 'profile' ? '#10b981' : 'rgba(148,163,184,0.8)', background: activeNav === 'profile' ? 'rgba(16,185,129,0.1)' : 'none', border: 'none', fontSize: '9px', padding: '8px 12px', borderRadius: '12px', cursor: 'pointer' }}><span style={{ fontSize: '20px', marginBottom: '4px' }}>👤</span><span>Profile</span></button>
        <button onClick={() => handleNav('qr')} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', color: activeNav === 'qr' ? '#10b981' : 'rgba(148,163,184,0.8)', background: activeNav === 'qr' ? 'rgba(16,185,129,0.1)' : 'none', border: 'none', fontSize: '9px', padding: '8px 12px', borderRadius: '12px', cursor: 'pointer' }}><span style={{ fontSize: '20px', marginBottom: '4px' }}>📷</span><span>QR</span></button>
        <button onClick={() => handleNav('alerts')} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', color: activeNav === 'alerts' ? '#10b981' : 'rgba(148,163,184,0.8)', background: activeNav === 'alerts' ? 'rgba(16,185,129,0.1)' : 'none', border: 'none', fontSize: '9px', padding: '8px 12px', borderRadius: '12px', cursor: 'pointer' }}><span style={{ fontSize: '20px', marginBottom: '4px' }}>🔔</span><span>Alerts</span></button>
        <button onClick={() => handleNav('settings')} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', color: activeNav === 'settings' ? '#10b981' : 'rgba(148,163,184,0.8)', background: activeNav === 'settings' ? 'rgba(16,185,129,0.1)' : 'none', border: 'none', fontSize: '9px', padding: '8px 12px', borderRadius: '12px', cursor: 'pointer' }}><span style={{ fontSize: '20px', marginBottom: '4px' }}>⚙️</span><span>Settings</span></button>
      </nav>
    </div>
  );
}
