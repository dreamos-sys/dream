 ...
 * Fitur:
 * - Form ceklis harian untuk toilet dan ruangan (42 items!)
...
export async function render({ container, user, supabase }) {
    return `
        <div class="max-w-6xl mx-auto p-4">
            ...
            <div id="panel-form" class="tab-panel">
                <form id="janitorIndoorForm" class="glass-card p-6 rounded-xl shadow-lg space-y-4">
                    ...
                    <div class="border-t border-slate-700 pt-4">
                        <h3 class="section-title text-teal-400 text-lg font-semibold mb-3">🚽 Toilet (11 Items)</h3>
                    ...
                    <!-- Bagian RUANGAN (31 area) -->
                    <div class="border-t border-slate-700 pt-4">
                        <h3 class="section-title text-teal-400 text-lg font-semibold mb-3">🏢 Ruangan & Area Umum (31 Items)</h3>
                        <div class="checkbox-grid grid grid-cols-2 md:grid-cols-4 gap-2">
                            ...
                            <label class="flex items-center space-x-2"><input type="checkbox" id="ruang_kepsek" class="accent-teal-500"> <span>Kepala Sekolah</span></label>                            <label class="flex items-center space-x-2"><input type="checkbox" id="ruang_guru_laki" class="accent-teal-500"> <span>R. Guru (Laki)</span></label>
            ...
            <!-- Panel Riwayat -->
            <div id="panel-history" class="tab-panel hidden">                <div class="glass-card p-6 rounded-xl shadow-lg">
...
export async function afterRender({ user, supabase }) {
    ...
    function activateTab(tabId) {
        ...
        if (activeBtn) {            activeBtn.classList.add('active', 'border-teal-500', 'text-white');
    ...
    form?.addEventListener('submit', async (e) => {
        ...
        // Kumpulkan data checkbox toilet (11 items)
        const toiletItems = [
        ...
        // Kumpulkan data checkbox ruangan (31 items)
        const ruangItems = [
    ...
    async function loadHistory() {
        ...
        try {
            const { data, error } = await supabase
                .from('janitor_indoor_reports')                .select('*')
            ...
            data.forEach(item => {
                html += `
                    <tr class="border-b border-slate-700">
                        ...
                        <td class="px-2 py-2">
                            <span class="px-2 py-1 rounded-full text-xs ${item.status === 'selesai' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-yellow-500/20 text-yellow-400'}">${item.status || 'pending'}</span>
                        </td>
                        <td class="px-2 py-2">
                            <button onclick="alert('Detail ID: ${item.id}\\n\\nToilet: ${JSON.stringify(item.toilet)}\\nRuangan: ${JSON.stringify(item.ruangan)}')" class="text-blue-400 text-xs hover:underline">Detail</button>
                        </td>
                    </tr>
                `;
            });
...
export function cleanup() {    console.log('🧹 [JANITOR-INDOOR] Module cleanup');
}
