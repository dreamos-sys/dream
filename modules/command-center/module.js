...
export default async function({ container, supabase, user }) {
    ...
    const renderShell = () => {
        ...
        <div class="animate-fade-in" style="padding: 20px; color: #e2e8f0; font-family: 'Inter', sans-serif; max-width: 1400px; margin: 0 auto;">
            ...
            <div id="cmd-reminder-modal" class="hidden fixed inset-0 z-[9999] flex items-center justify-center" style="background: rgba(0,0,0,0.8); backdrop-filter: blur(5px);">
                <div style="background: rgba(15, 23, 42, 0.95); border: 1px solid #3b82f6; border-radius: 16px; padding: 25px; width: 100%; max-width: 400px;">
    ...
    const loadDashboard = async () => {
        ...
        vp.innerHTML = `
            ...
            <div id="monitor-card" style="background: rgba(15, 23, 42, 0.8); border: 1px solid rgba(255,255,255,0.1); border-radius: 16px; padding: 25px; text-align: center; position: relative; overflow: hidden; margin-bottom: 20px;">
                ...
                <div id="monitor-progress" style="position: absolute; bottom: 0; left: 0; height: 3px; background: #10b981; width: 0%;"></div>
            </div>
    ...
    const loadDanaList = async () => {
        ...
        try {
            const { data } = await supabase.from(CONFIG.tables.dana).select('*').order('created_at', { ascending: false }).limit(20);
            vp.innerHTML = `
    ...
    const runMonitor = () => {
        ...
        setTimeout(() => {
            progEl.style.transition = 'width 7s linear'; progEl.style.width = '100%';
            valEl.textContent = globalStats[slide.id] || '0';
    ...
    const initChart = () => {
        ...
        setTimeout(async () => {
            ...
            // Fake data for enterprise look, or real data if requested
            new Chart(ctx, {
                type: 'line',
    ...
    window.cmdGenQR = async () => {
        ...
        // We embed basic info into QR for security/speed
    ...
    document.getElementById('cmd-reminder-form').addEventListener('submit', async (e) => {
        e.preventDefault();
        const data = {
            nama_item: document.getElementById('rem_nama').value,
            lokasi: document.getElementById('rem_lokasi').value,
