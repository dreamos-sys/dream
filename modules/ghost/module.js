        ...
        <div class="ghost-architect-panel" style="padding: 1rem; max-width: 1400px; margin: 0 auto;">
            ...
            ">
                ...
                <div style="display: flex; align-items: center; gap: 1rem; margin-bottom: 1rem; position: relative;">
                    ...
                    <div>
                        <h2 class="text-2xl font-bold" style="color: var(--dream-primary); margin: 0;">
                            Ghost Architect Mode
                        </h2>
                        <p class="text-sm" style="color: var(--dream-text-muted); margin: 0;">
                ...
                <div style="display: flex; gap: 0.5rem; flex-wrap: wrap; position: relative;">                    <button onclick="window.ghostExportData()" class="btn-ghost" style="
            ...
            <div class="ghost-tabs" style="
                ...
                overflow-x: auto;
            ">
                <button class="ghost-tab active" data-tab="overview" style="                    padding: 0.75rem 1.25rem;
                    background: rgba(16, 185, 129, 0.15);
                    border: 1px solid rgba(16, 185, 129, 0.3);
                ...
                <button class="ghost-tab" data-tab="tools" style="
                    ...
                    border: 1px solid transparent;
                    border-radius: 8px 8px 0 0;
                    color: var(--dream-text-muted);                    font-weight: 600;
                    cursor: pointer;
                ">🛠️ Tools</button>
            ...
            <!-- ════════════════════════════════════
                 FOOTER
                 ════════════════════════════════════ -->
            <div class="text-center" style="margin-top: 1.5rem; padding: 1rem; color: var(--dream-text-subtle); font-size: 0.75rem;">
    ...
    document.querySelectorAll('.ghost-tab').forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            document.querySelectorAll('.ghost-tab').forEach(t => {
                ...
                t.style.color = 'var(--dream-text-muted)';            });
...
function renderOverview(info, modules, performance) {
    return `
        <h3 class="text-lg font-semibold mb-4" style="color: var(--dream-primary);">📊 System Overview</h3>
        ...
        <div class="grid grid-cols-2 gap-4 mb-6" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 1rem;">
            ${Object.entries(info).map(([key, value]) => `
                <div class="info-item" style="                    background: rgba(255, 255, 255, 0.03);
                    border: 1px solid rgba(148, 163, 184, 0.15);
                    border-radius: 8px;
                    padding: 0.75rem;
                ">
                    <div style="font-size: 0.75rem; color: var(--dream-text-subtle); text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 0.25rem;">
                        ${key.replace(/([A-Z])/g, ' $1').trim()}
                    </div>
                    <div style="font-family: monospace; font-size: 0.9rem; color: var(--dream-text); word-break: break-all;">
                        ${value}
                    </div>
                </div>
            `).join('')}
        ...
        ">
            ...
            <div style="display: flex; flex-wrap: wrap; gap: 0.5rem;">
                ${modules.length > 0 ? modules.map(mod => `
                    ...
                    ">${mod}</span>
                `).join('') : '<span style="color: var(--dream-text-muted);">No modules loaded</span>'}
            </div>
        </div>
        ...
        ">
            ...
            <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 1rem; text-align: center;">
                <div>
                    ...
                    <div style="font-size: 0.75rem; color: var(--dream-text-subtle);">Used</div>
                </div>
                <div>                    <div style="font-size: 1.5rem; font-weight: bold; color: #3b82f6;">${performance.total} MB</div>
                    <div style="font-size: 0.75rem; color: var(--dream-text-subtle);">Total</div>
                </div>
                <div>
                    <div style="font-size: 1.5rem; font-weight: bold; color: #8b5cf6;">${performance.limit} MB</div>
                    <div style="font-size: 0.75rem; color: var(--dream-text-subtle);">Limit</div>
                </div>
            </div>
        </div>
        ` : ''}
...
function renderLogs(logs) {
    return `
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
            <h3 class="text-lg font-semibold" style="color: var(--dream-primary);">📋 System Logs (${logs.length})</h3>
        ...
        <pre style="
            ...
            line-height: 1.6;        ">${logs.map(log => `[${log.timestamp || 'N/A'}] <span style="color: ${log.level === 'ERROR' ? '#ef4444' : log.level === 'WARN' ? '#f59e0b' : '#10b981'}">${log.level}</span>: ${log.message}`).join('\n') || 'No logs available'}</pre>
    `;
}
...
function renderErrors(errors) {
    return `
        ...
        ${errors.length === 0 ? `
            <div class="text-center py-10" style="color: var(--dream-text-muted);">
                ...
                ${errors.map((error, index) => `
                    ...
                    ">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem;">
                            ...
                            <span style="font-size: 0.75rem; color: var(--dream-text-subtle);">${error.timestamp || 'N/A'}</span>
                        </div>
                        <div style="font-family: monospace; font-size: 0.85rem; color: var(--dream-text); word-break: break-all;">
                            ${error.message}
                        </div>
                    </div>
                `).join('')}
    ...
    return `
        ...
        <div class="grid grid-cols-2 gap-4" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 1rem;">
            <div class="glass-card p-4" style="background: rgba(6, 182, 212, 0.1); border: 1px solid rgba(6, 182, 212, 0.2); border-radius: 12px; padding: 1rem;">
                ...
                ` : '<span style="color: var(--dream-text-muted);">Not available</span>'}            </div>
...
function renderSecurity() {
    return `
        ...
        <div style="display: flex; flex-direction: column; gap: 0.75rem;">
            ...
            ">
                <div>
                    ...
                    <div style="font-size: 0.75rem; color: var(--dream-text-subtle);">Unique device identification</div>                </div>
            ...
            ">
                <div>
                    ...
                    <div style="font-size: 0.75rem; color: var(--dream-text-subtle);">Face/Touch ID support</div>
                </div>
            ...
            ">
                <div>
                    ...
                    <div style="font-size: 0.75rem; color: var(--dream-text-subtle);">Location-based access</div>
                </div>
                <span style="background: rgba(16, 185, 129, 0.2); color: #10b981; padding: 0.25rem 0.75rem; border-radius: 20px; font-size: 0.75rem;">✅ Active</span>
            </div>
            ...
            ">
                <div>
                    ...
                    <div style="font-size: 0.75rem; color: var(--dream-text-subtle);">Complete activity logging</div>
                </div>
                <span style="background: rgba(16, 185, 129, 0.2); color: #10b981; padding: 0.25rem 0.75rem; border-radius: 20px; font-size: 0.75rem;">✅ Logging</span>            </div>
        </div>
        ...
        ">
            ...
            <ul style="font-size: 0.85rem; color: var(--dream-text); line-height: 1.8;">
    ...
    return `
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 1rem;">
            ...
            " onmouseover="this.style.background='rgba(16, 185, 129, 0.25)'; this.style.transform='translateY(-2px)'" onmouseout="this.style.background='rgba(16, 185, 129, 0.15)'; this.style.transform='translateY(0)'">
                ...
                <div style="font-size: 0.75rem; color: var(--dream-text-subtle); margin-top: 0.25rem;">Clear all cached data</div>
            </button>
            ...
            " onmouseover="this.style.background='rgba(6, 182, 212, 0.25)'; this.style.transform='translateY(-2px)'" onmouseout="this.style.background='rgba(6, 182, 212, 0.15)'; this.style.transform='translateY(0)'">
                <div style="font-size: 2rem; margin-bottom: 0.5rem;">🔄</div>                <div style="font-weight: 600; color: #06b6d4;">Reload App</div>
                <div style="font-size: 0.75rem; color: var(--dream-text-subtle); margin-top: 0.25rem;">Refresh application</div>
            </button>
            ...
            " onmouseover="this.style.background='rgba(139, 92, 246, 0.25)'; this.style.transform='translateY(-2px)'" onmouseout="this.style.background='rgba(139, 92, 246, 0.15)'; this.style.transform='translateY(0)'">
                ...
                <div style="font-size: 0.75rem; color: var(--dream-text-subtle); margin-top: 0.25rem;">Download system data</div>
            </button>
            ...
            " onmouseover="this.style.background='rgba(245, 158, 11, 0.25)'; this.style.transform='translateY(-2px)'" onmouseout="this.style.background='rgba(245, 158, 11, 0.15)'; this.style.transform='translateY(0)'">
                ...
                <div style="font-size: 0.75rem; color: var(--dream-text-subtle); margin-top: 0.25rem;">Export error logs</div>
            </button>
            ...
            " onmouseover="this.style.background='rgba(59, 130, 246, 0.25)'; this.style.transform='translateY(-2px)'" onmouseout="this.style.background='rgba(59, 130, 246, 0.15)'; this.style.transform='translateY(0)'">
                ...
                <div style="font-size: 0.75rem; color: var(--dream-text-subtle); margin-top: 0.25rem;">System health check</div>
            </button>
            ...
            " onmouseover="this.style.background='rgba(239, 68, 68, 0.25)'; this.style.transform='translateY(-2px)'" onmouseout="this.style.background='rgba(239, 68, 68, 0.15)'; this.style.transform='translateY(0)'">
                ...
                <div style="font-size: 0.75rem; color: var(--dream-text-subtle); margin-top: 0.25rem;">Hide/Show panel</div>
            </button>
        </div>
    `;
}
...
function renderSettings() {
    return `
        ...
        <div style="display: flex; flex-direction: column; gap: 1rem;">
            ...
            ">
                <div>
                    <div style="font-weight: 600; color: var(--dream-text);">Debug Mode</div>
                    <div style="font-size: 0.75rem; color: var(--dream-text-subtle);">Enable detailed logging</div>
                </div>
            ...
            ">
                <div>
                    <div style="font-weight: 600; color: var(--dream-text);">Auto-Clear Logs</div>
                    <div style="font-size: 0.75rem; color: var(--dream-text-subtle);">Clear logs after 7 days</div>
                </div>                <label style="position: relative; display: inline-block; width: 50px; height: 26px;">
            ...
            ">
                <div>
                    <div style="font-weight: 600; color: var(--dream-text);">Error Notifications</div>
                    <div style="font-size: 0.75rem; color: var(--dream-text-subtle);">Show toast on errors</div>
                </div>
                <label style="position: relative; display: inline-block; width: 50px; height: 26px;">
                    <input type="checkbox" checked style="opacity: 0; width: 0; height: 0;">
                    <span style="position: absolute; cursor: pointer; top: 0; left: 0; right: 0; bottom: 0; background-color: #10b981; border-radius: 34px; transition: .4s;"></span>
...
window.ghostRunDiagnostics = function() {
    const diagnostics = {
        ...
        camera: navigator.mediaDevices ? 'OK' : 'Not Available',        biometric: window.PublicKeyCredential ? 'OK' : 'Not Available',
